...
import { getDefaultDataDir, getSessionsDir } from "./paths.js";
import { readFirstLineSync, readLinesAsBuffers } from "./file-lines.js";
import {
...
// Entry types that can represent user intent rather than lifecycle/status bookkeeping.
// Used by hasUserContent to decide whether a message-less draft is safe to discard.
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, undefined/false if core-generated (backward compatible) */
	fromHook?: boolean;
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, false if core-generated */
	fromHook?: boolean;
}
...
export function parseSessionEntries(content: string): FileEntry[] {
	...
	for (const line of lines) {
		...
		try {
			...
			reportOperationalError({ component: "sessions", operation: "parse session JSONL entry", error, severity: "warn" });
		}
	}
...
export function buildSessionContext(
	...
	if (compaction) {
		...
		const retainedMessages: SessionMessage[] = [];
 ...
 */
export function getDefaultSessionDir(_cwd: string, dataDir: string = getDefaultDataDir()): string {
	const sessionDir = getSessionsDir(dataDir);
...
function appendEntryFromBuffer(entries: FileEntry[], buffer: Buffer, start = 0, end = buffer.length): void {
	...
	try {
		...
		if (end > start) reportOperationalError({ component: "sessions", operation: "parse buffered session JSONL entry", error, severity: "warn" });
	}
}
...
// Async loader: reads off the event loop and yields while parsing so a
...
function isValidSessionFile(filePath: string): boolean {
	try {
		...
		reportOperationalError({ component: "sessions", operation: `validate session file ${filePath}`, error, severity: "warn" });
		return false;
	}
}
...
export function findMostRecentSession(sessionDir: string): string | null {
	try {
		...
		reportOperationalError({ component: "sessions", operation: `find most recent session in ${sessionDir}`, error });
		return null;
	}
}
...
export function findMostRecentSessionForCwd(sessionDir: string, cwd: string): string | null {
	try {
		const files = readdirSync(sessionDir)
			...
			.map((path) => {
				try {
					...
					reportOperationalError({ component: "sessions", operation: `inspect session header ${path}`, error, severity: "warn" });
					return undefined;
				}
			})
		...
		reportOperationalError({ component: "sessions", operation: `find recent session for workspace ${cwd}`, error });
		return null;
	}
}

function isMessageWithContent(message: SessionMessage): message is Message {
...
export async function readSessionInfo(filePath: string): Promise<SessionInfo | null> {
	...
	try {
		...
		if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
			reportOperationalError({ component: "sessions", operation: `stat session ${filePath}`, error, severity: "warn" });
		}
		return null;
	}
...
async function scanSessionInfo(filePath: string, stats: Awaited<ReturnType<typeof stat>>): Promise<SessionInfo | null> {
	try {
		...
		for await (const lineBuffer of readLinesAsBuffers(filePath)) {
			...
			// Large entries can be many MB. They do not carry the
			...
			try {
				...
				reportOperationalError({ component: "sessions", operation: `scan session entry in ${filePath}`, error, severity: "warn" });
				continue;
			}
		...
		reportOperationalError({ component: "sessions", operation: `scan session metadata ${filePath}`, error });
		return null;
	}
}
...
async function listSessionsFromDir(
	...
	try {
		...
		reportOperationalError({ component: "sessions", operation: `list sessions in ${dir}`, error });
	}
...
export class SessionManager {
	 ...
	 * lets async callers skip the synchronous re-read.
	 */
	setSessionFile(sessionFile: string, preloadedEntries?: FileEntry[]): void {
		this.sessionFile = resolve(sessionFile);
		if (existsSync(this.sessionFile)) {
			ensurePrivateSessionFile(this.sessionFile);
			const existingSize = statSync(this.sessionFile).size;
			this.fileEntries = preloadedEntries ?? loadEntriesFromFile(this.sessionFile);

			// An actually empty file can be initialized in place. A non-empty file with no
			// valid session header is corruption, not an empty session. Preserve it for
			// recovery/debugging before creating a clean replacement so a parser failure
			// can never silently destroy the user's transcript.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (existingSize > 0) {
					const suffix = new Date().toISOString().replace(/[:.]/g, "-");
					const quarantinePath = `${explicitPath}.corrupt-${suffix}-${randomUUID()}`;
					renameSync(explicitPath, quarantinePath);
					reportOperationalError({
						component: "sessions",
						operation: "quarantine corrupted session",
						error: new Error(`Invalid session file preserved at ${quarantinePath}`),
						severity: "warn",
					});
				}
				this.newSession();
	...
	private _notifyPersistListeners(): void {
		...
		for (const listener of this.persistListeners) {
			try {
				...
				reportOperationalError({ component: "sessions", operation: "notify persistence observer", error });
			}
		}
	}
	...
	_persist(entry: SessionEntry): void {
		...
		if (!this.flushed || !existsSync(this.sessionFile)) {
			...
			ensurePrivateDirectory(dirname(this.sessionFile));
			ensurePrivateSessionFile(this.sessionFile);
	...
	private _appendEntry(entry: SessionEntry): void {
		...
		try {
			...
			// Keep the in-memory graph exactly aligned with durable history. Otherwise a
			// later entry could point at a parent that was never successfully persisted.
			this.byId.delete(entry.id);
			const index = this.fileEntries.findIndex((candidate) => candidate === entry);
			if (index >= 0) this.fileEntries.splice(index, 1);
			this.leafId = previousLeafId;
			this.flushed = false;

			// appendFileSync can fail after a partial write. Rebuild from the rolled-back
			// graph when possible; if storage is still unavailable, force the next write
			// to perform a full rewrite and surface the original error to the caller.
			try {
				this.flushNow();
			} catch (rewriteError) {
				reportOperationalError({
					component: "sessions",
					operation: "rewrite session after failed append",
					error: rewriteError,
				});
				this.flushed = false;
			}
	 ...
	 * only lifecycle bookkeeping (session_state) or
	 * the default model/thinking entries every new session is created with. Used by
	 * discard guards to decide whether a message-less draft is safe to
	 * delete (that guard always also requires zero messages).
	 *
	 * session initialization may add an optional leading `model_change`
	...
	private _appendEntryWithRollback(append: () => string): string {
		...
		try {
			...
			// _appendEntry already rolls back failures from its own persistence call. The
			// extra rollback here covers a later flushNow() failure.
			if (this.leafId !== null && this.leafId !== previousLeafId) {
				...
				try {
					...
					reportOperationalError({ component: "sessions", operation: "rewrite session after failed append", error: rewriteError });
					this.flushed = false;
				}
			}
			throw error;
	...
	/**
	 * Non-blocking open(): parses off the event loop so a large load
