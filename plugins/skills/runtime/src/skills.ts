...
function addIgnoreRules(ig: IgnoreMatcher, dir: string, rootDir: string): void {
	...
	for (const filename of IGNORE_FILE_NAMES) {
		...
		try {
			...
			reportOperationalError({ component: "skills", operation: `read ignore rules ${ignorePath}`, error, severity: "warn" });
		}
	}
}
...
function loadSkillsFromDirInternal(
	...
	try {
		...
		reportOperationalError({ component: "skills", operation: `discover skills in ${dir}`, error, severity: "warn" });
	}
...
export interface LoadSkillsOptions {
	/** Working directory used to resolve relative explicit skill paths. */
	cwd: string;
	/** Explicit skill paths (files or directories). */
	skillPaths: string[];
	/** Include caller-supplied default user/project skill roots. */
	includeDefaults: boolean;
	/** Optional default user skill directory. Configuration ownership stays outside this runtime. */
	userSkillsDir?: string;
	/** Optional default project skill directory. Configuration ownership stays outside this runtime. */
...
export function loadSkills(options: LoadSkillsOptions): LoadSkillsResult {
	const { cwd, skillPaths, includeDefaults, userSkillsDir, projectSkillsDir } = options;
	...
	const pythonImportDiagnostics: SkillDiagnostic[] = [];
	...
	if (includeDefaults) {
		if (userSkillsDir) {
			addSkills(loadSkillsFromDirInternal(resolve(userSkillsDir), "user", true));
		}
		if (projectSkillsDir) {
			addSkills(loadSkillsFromDirInternal(resolve(projectSkillsDir), "project", true));
		}
	}

	const resolvedUserSkillsDir = userSkillsDir ? resolve(userSkillsDir) : undefined;
	const resolvedProjectSkillsDir = projectSkillsDir ? resolve(projectSkillsDir) : undefined;
	...
	const getSource = (resolvedPath: string): "user" | "project" | "path" => {
		if (resolvedUserSkillsDir && isUnderPath(resolvedPath, resolvedUserSkillsDir)) return "user";
		if (resolvedProjectSkillsDir && isUnderPath(resolvedPath, resolvedProjectSkillsDir)) return "project";
		return "path";
	};
