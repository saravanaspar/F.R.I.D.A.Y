...
function settingsFromParsed(parsed: Partial<Record<SupportedKey, string>>): RuntimeSettings | undefined {
...
export async function readRuntimeSettings(home = getFridayHome()): Promise<RuntimeSettings | undefined> {
  return settingsFromParsed(await readParsedRuntimeEnvironment(resolve(home)));
}
...
export async function saveRuntimeSettings(
  ...
  const contents = serializeRuntimeSettings(settings);
