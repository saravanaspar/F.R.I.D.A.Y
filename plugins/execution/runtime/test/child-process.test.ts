...
import { isProcessAlive, isZombieProcess, launchDetachedProcess, waitForChildProcess } from "../src/process/child-process.js";
...
describe("detached process launch", () => {
	it("launches a detached child only after spawn succeeds", async () => {
		const child = await launchDetachedProcess(process.execPath, ["--eval", "setTimeout(() => process.exit(0), 25)"], {
			env: process.env,
		});
		expect(child.pid > 0).toBe(true);
	});

	it("rejects when the detached executable cannot be spawned", async () => {
		await expect(
			launchDetachedProcess("friday-this-executable-does-not-exist", [], { env: process.env }),
		).rejects.toThrow();
	});
});
