...
export interface DetachedProcessLaunchOptions {
	cwd?: string;
	env?: NodeJS.ProcessEnv;
}

export interface DetachedProcessLaunchResult {
	pid: number;
}

/** Launch a detached child and resolve only after the OS accepted the spawn. */
export function launchDetachedProcess(
	command: string,
	args: readonly string[],
	options: DetachedProcessLaunchOptions = {},
): Promise<DetachedProcessLaunchResult> {
	return new Promise((resolveLaunch, rejectLaunch) => {
		...
		const onSpawn = () => {
			...
			const pid = child.pid;
			if (!pid) {
				rejectLaunch(new Error("Detached child did not expose a pid"));
				return;
			}
			child.unref();
			resolveLaunch({ pid });
...
export function isZombieProcess(pid: number): boolean {
	...
	try {
		...
		// friday-expected-control-flow: /proc is an optional fast probe; use ps next.
	}
	try {
		const state = execFileSync("ps", ["-p", String(pid), "-o", "stat="], { encoding: "utf8" }).trim();
		return state.startsWith("Z");
	} catch {
		// friday-expected-control-flow: an absent ps row means the process is not a zombie.
		return false;
	}
}
...
export function signalProcessGroupOrProcess(pid: number, signal: NodeJS.Signals): void {
	try {
		...
		// friday-expected-control-flow: fall back when process groups are unavailable.
	}
	try {
		process.kill(pid, signal);
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code !== "ESRCH") {
			reportOperationalError({ component: "execution.process", operation: `signal process ${pid}`, error, severity: "warn" });
		}
	}
}
