...
const RESULT_MARKER = "__FRIDAY_KERNEL_STATE__";
...
def _friday_snapshot_state():
    ...
    # Interpreter/runtime handles are recreated on every start; never snapshot them.
    always_skip = {"asyncio", "In", "Out", "get_ipython", "exit", "quit", "open"}
...
try:
    _friday_snapshot_state()
finally:
    del _friday_snapshot_state
`.trim();
}
...
try:
    _friday_restore_state()
finally:
    del _friday_restore_state
`.trim();
}
...
try:
    _friday_list_state_names()
finally:
    del _friday_list_state_names
`.trim();
}
