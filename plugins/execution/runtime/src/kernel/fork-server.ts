...
// alive for the host process. Each KernelManager asks it to fork a kernel onto a
...
// FRIDAY_KERNEL_FORKSERVER=0 opts out.
export function isForkServerEnabled(): boolean {
	if (process.platform !== "linux") return false;
	return process.env.FRIDAY_KERNEL_FORKSERVER !== "0";
}
...
class ForkServer {
	...
	private start(): Promise<void> {
		this.socketDir = mkdtempSync(join(tmpdir(), "friday-kernel-forkserver-"));
	...
	private onData(chunk: string): void {
		...
		for (let idx = this.buffer.indexOf("\n"); idx !== -1; idx = this.buffer.indexOf("\n")) {
			...
			try {
				...
				reportOperationalError({ component: "execution.kernel", operation: "parse forkserver protocol message", error });
				continue;
			}
			if (msg.type === "ready") {
				this.onReady();
				...
				if (this.abandoned.delete(msg.id) && typeof msg.pid === "number") {
					try {
						...
						if ((error as NodeJS.ErrnoException).code !== "ESRCH") {
							reportOperationalError({ component: "execution.kernel", operation: "terminate abandoned forked kernel", error, severity: "warn" });
						}
					}
				}
				continue;
	...
	dispose(reason = "forkserver disposed"): void {
		...
		try {
			...
			reportOperationalError({ component: "execution.kernel", operation: "destroy forkserver connection", error, severity: "warn" });
		}
		try {
			this.server?.close();
		} catch (error) {
			reportOperationalError({ component: "execution.kernel", operation: "close forkserver listener", error, severity: "warn" });
		}
		try {
			this.proc?.kill("SIGTERM");
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code !== "ESRCH") {
				reportOperationalError({ component: "execution.kernel", operation: "terminate forkserver process", error, severity: "warn" });
			}
		}
		if (this.socketDir) {
			try {
				...
				reportOperationalError({ component: "execution.kernel", operation: "remove forkserver socket directory", error, severity: "warn" });
			}
