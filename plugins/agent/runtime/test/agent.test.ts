...
function createTestModel(provider = "test", id = "test-model"): Model<string> {
	return {
		id,
		name: id,
		api: "test",
...
describe("Agent", () => {
	...
	it("should create an agent instance with custom initial state", () => {
		const customModel = createTestModel("openai", "gpt-4o-mini");
		const agent = new Agent({
			initialState: {
	...
	it("should update state with mutators", () => {
		...
		// Test setModel
		const newModel = createTestModel("google", "gemini-2.5-flash");
	...
	it("should not drain steering messages when aborting before a queued poll", async () => {
		...
		const config: AgentLoopConfig = {
			model: createTestModel("openai", "gpt-4o-mini"),
			convertToLlm: () => [],
			getSteeringMessages,
		};
