...
import { createAssistantMessageDiagnostic, streamSimple } from "./model-access.js";
import type {
	ImageContent,
	Message,
	Model,
...
import { FRIDAY_MODEL_RETRY_MAX_RETRIES } from "./retry.js";
import type {
...
function createMutableAgentState(
	...
	return {
		...
		stableSystemPromptPrefix: initialState?.stableSystemPromptPrefix,
...
export class Agent {
	...
	/** High-level retry budget for transient provider/model request failures. */
	public modelRetryMaxRetries?: number;
	...
	constructor(options: AgentOptions = {}) {
		...
		this.modelRetryMaxRetries = options.modelRetryMaxRetries ?? FRIDAY_MODEL_RETRY_MAX_RETRIES;
		// Safety-first default: models frequently emit dependent filesystem/process/external
		// mutations in one assistant message. Parallelism is opt-in for callers that can
		// prove the batch is independent. Individual tools may still advertise parallel
		// capability when the host chooses a parallel agent mode explicitly.
	...
	private createContextSnapshot(): AgentContext {
		...
		if (this._state.stableSystemPromptPrefix !== undefined) {
			context.stableSystemPromptPrefix = this._state.stableSystemPromptPrefix;
		}
		return context;
	}
	...
	private createLoopConfig(options: { skipInitialSteeringPoll?: boolean } = {}): AgentLoopConfig {
		...
		return {
			...
			modelRetryMaxRetries: this.modelRetryMaxRetries,
			...
			getStableSystemPromptPrefix: () => this._state.stableSystemPromptPrefix,
	...
	private async handleRunFailure(error: unknown, aborted: boolean): Promise<void> {
		...
		await this.processEvents({ type: "message_start", message: failureMessage }).catch((eventError: unknown) => {
			reportOperationalError({ component: "agent", operation: "publish lifecycle failure message_start", error: eventError });
		});
		await this.processEvents({ type: "message_end", message: failureMessage }).catch((eventError: unknown) => {
			reportOperationalError({ component: "agent", operation: "publish lifecycle failure message_end", error: eventError });
		});
		await this.processEvents({ type: "agent_end", messages: [failureMessage] }).catch((eventError: unknown) => {
			reportOperationalError({ component: "agent", operation: "publish lifecycle failure agent_end", error: eventError });
		});
	}
