...
export type StreamFn = (
	model: Model<any>,
	context: import("./model-types.js").Context,
...
export interface AgentLoopConfig extends SimpleStreamOptions {
	model: Model<any>;
	/** High-level model-request retries after provider/SDK retries are exhausted. */
	...
	/** Resolves the stable leading system-prompt prefix immediately before each LLM call. */
	getStableSystemPromptPrefix?: () => string | undefined;
 ...
 * thinking-level metadata from the model runtime to detect support for a concrete model.
 */
 ...
 *
 * Consumers can extend this interface with TypeScript declaration merging.
 */
...
export interface AgentState {
	...
	systemPrompt: string;
	/** Stable leading bytes providers may cache independently. */
...
export type AgentEvent =
	...
	// Public retry status. This is operational metadata, never hidden reasoning.
	| { type: "model_retry"; attempt: number; maxRetries: number; delayMs: number; message: string }
