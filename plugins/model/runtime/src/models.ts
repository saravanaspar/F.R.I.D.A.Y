...
/** Register a runtime model descriptor, used for user-configured compatible endpoints. */
...
export function unregisterModel(providerInput: string, modelIdInput: string): boolean {
	...
	if (providerModels.size === 0 && !(provider in MODELS)) modelRegistry.delete(provider);
	return removed;
}
...
export function calculateCost<TApi extends Api>(
	...
	usage.cost.estimated = usage.cost.total;
	usage.cost.source = "catalog-estimate";
	return usage.cost;
}
