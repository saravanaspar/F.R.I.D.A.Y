 ...
 * Defaults to "short" and uses FRIDAY_CACHE_RETENTION for backward compatibility.
 */
...
function buildParams(
	...
	if (isOAuthToken) {
		...
		// Keep the canonical system prompt as one stored string, but split a
		// host-declared stable prefix into request-local cache blocks. With the
		// existing last-tool + latest-user markers this stays within Anthropic's
		// four-breakpoint cap: stable prefix, volatile suffix, tools, user.
		const stableSplit = cacheControl
			? splitStableSystemPrompt(context.systemPrompt, context.stableSystemPromptPrefix)
			: undefined;
		if (stableSplit) {
