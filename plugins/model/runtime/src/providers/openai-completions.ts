...
import { splitStableSystemPrompt } from "../cache-semantics.js";
...
import { hostnameMatches, normalizedUrlHostname, urlHostnameMatches } from "./url-security.js";
...
function resolveCacheRetention(cacheRetention?: CacheRetention): CacheRetention {
	...
	if (typeof process !== "undefined" && process.env.FRIDAY_CACHE_RETENTION === "long") {
		return "long";
	}
	return "short";
}
...
function buildParams(
	...
	if (cacheControl) {
		applyAnthropicCacheControl(messages, params.tools, cacheControl, context);
	}
	...
	if (urlHostnameMatches(model.baseUrl, "openrouter.ai") && model.compat?.openRouterRouting) {
...
function applyAnthropicCacheControl(
	...
	addCacheControlToSystemPrompt(messages, cacheControl, context);
...
function addCacheControlToSystemPrompt(
	...
	for (const message of messages) {
		...
		const stableSplit = splitStableSystemPrompt(context.systemPrompt, context.stableSystemPromptPrefix);
		if (stableSplit && typeof message.content === "string") {
			const [stablePrefix, volatileSuffix] = stableSplit;
			message.content = [
				{ type: "text", text: sanitizeSurrogates(stablePrefix), cache_control: cacheControl },
				{
					type: "text",
					text: sanitizeSurrogates(volatileSuffix),
...
function parseChunkUsage(
	rawUsage: {
		...
		/** OpenRouter returns the amount charged in every final usage payload. */
		cost?: number | string;
		cost_details?: { upstream_inference_cost?: number | string; currency?: string };
	},
	model: Model<"openai-completions">,
	...
	const providerCost = typeof rawUsage.cost === "number" ? rawUsage.cost : Number(rawUsage.cost);
	if (rawUsage.cost !== undefined && Number.isFinite(providerCost) && providerCost >= 0) {
		usage.cost.actual = providerCost;
		usage.cost.total = providerCost;
		usage.cost.source = "provider-reported";
		usage.cost.currency = rawUsage.cost_details?.currency?.trim().toUpperCase()
			|| (urlHostnameMatches(model.baseUrl, "openrouter.ai") ? "USD" : undefined);
	}
	return usage;
}
...
function detectCompat(model: Model<"openai-completions">): ResolvedOpenAICompletionsCompat {
	...
	const baseHostname = normalizedUrlHostname(baseUrl);
	const hostIs = (expectedDomain: string) => hostnameMatches(baseHostname, expectedDomain);

	const isZai = provider === "zai" || hostIs("api.z.ai");
	const isMoonshot =
		provider === "moonshotai" ||
	...
	const cacheControlFormat =
		isAnthropicModel && (provider === "openrouter") ? "anthropic" : undefined;
