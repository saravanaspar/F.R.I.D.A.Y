	...
	(async () => {
		...
		try {
			...
			let commandInput = {
				...
				system: buildSystemPrompt(context.systemPrompt, context.stableSystemPromptPrefix, model, cacheRetention),
				inferenceConfig: {
...
function buildSystemPrompt(
	...
	stableSystemPromptPrefix: string | undefined,
	model: Model<"bedrock-converse-stream">,
	cacheRetention: CacheRetention,
): SystemContentBlock[] | undefined {
	if (!systemPrompt) return undefined;

	const cachingEnabled = cacheRetention !== "none" && supportsPromptCaching(model);
	if (!cachingEnabled) return [{ text: sanitizeSurrogates(systemPrompt) }];

	const cachePoint: SystemContentBlock = {
		cachePoint: { type: CachePointType.DEFAULT, ...(cacheRetention === "long" ? { ttl: CacheTTL.ONE_HOUR } : {}) },
	};
	const stableSplit = splitStableSystemPrompt(systemPrompt, stableSystemPromptPrefix);
	if (stableSplit) {
	...
	return [{ text: sanitizeSurrogates(systemPrompt) }, cachePoint];
}
