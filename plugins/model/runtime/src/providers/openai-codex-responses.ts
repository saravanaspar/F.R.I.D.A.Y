...
import { stripTrailingSlashes } from "./url-security.js";
...
async function* parseSSE(response: Response): AsyncGenerator<Record<string, unknown>> {
	...
	try {
		...
		try {
			...
			reportOperationalError({ component: "model.openai-codex", operation: "cancel response reader", error, severity: "warn" });
		}
		try {
			reader.releaseLock();
		} catch (error) {
			reportOperationalError({ component: "model.openai-codex", operation: "release response reader lock", error, severity: "warn" });
		}
	}
}
...
function closeWebSocketSilently(socket: WebSocketLike, code = 1000, reason = "done"): void {
	try {
		...
		reportOperationalError({ component: "model.openai-codex", operation: "close cached websocket", error, severity: "warn" });
	}
}
...
async function parseErrorResponse(response: Response): Promise<{ message: string; friendlyMessage?: string }> {
	...
	try {
		...
		// friday-expected-control-flow: a provider error body may be plain text.
	}
