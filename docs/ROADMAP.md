...
Status: **in progress**. The provider-neutral Computer core defines `ComputerNodeAdapter`, resource admission with `WAITING_FOR_COMPUTER`, expiring `ScreenLease` and generation-bound `ControlLease` records, Browser Supervisor snapshots/action fallback ordering, secret-safe human takeover, automatic/manual hand-back with mandatory re-observation, and guarded provider lifecycle operations. The second slice now activates Phase 3 `computer:<node-id>` targets: Projects may select them, Turn Loop leases the matching Agent screen, and existing bash/edit/process/IPython tools dispatch through the Computer provider under their existing Permissions checks and the current control generation. Durable Session Job waiting/resume, Agent/browser Computer tools, Client Gateway Computer APIs, Browser Supervisor orchestration, and the complete takeover/resume acceptance path remain before Phase 4 is complete. Linux/Sway and Windows platform implementations remain in Phases 5 and 8.
