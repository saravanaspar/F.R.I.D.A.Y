...
Acceptance: a Pi user works on the physical monitor while two Agents browse independently with shared filesystem and browser login state. The Linux provider must also pass Computer conformance coverage proving that passwords, OTPs, human keystrokes, CAPTCHA contents, and sensitive screenshots are omitted before a provider marks an observation safe for Agent/Client exposure.
