...
sections carry typed authority and cache scope; direct user/profile/Skill
configuration cannot outrank core/host policy, repository `AGENTS.md` is bounded
project guidance, external/tool/UI content stays untrusted data, and compaction
cannot promote that data into durable user instructions. Turn Loop supplies
canonical time/timezone and active model image capability as runtime facts.
Conditional-hook phases are host-validated, sibling Project mutations are
coordinated over the intentionally shared workspace, and each concurrent Agent
continues to receive its own Computer screen while sharing browser/account/file
state at the Computer Node level.
...
Status: **complete**. The provider-neutral Computer core defines `ComputerNodeAdapter`, resource admission with `WAITING_FOR_COMPUTER`, expiring `ScreenLease` and generation-bound `ControlLease` records, Browser Supervisor ownership/readiness invariants and action fallback ordering, secret-safe human takeover, automatic/manual hand-back with mandatory re-observation, and guarded provider lifecycle operations. Phase 3 `computer:<node-id>` targets execute through the Computer capability: Projects select the target, Turn Loop leases the matching Agent screen, renews that lease for the active run even during manual-only takeover, and existing bash/edit/process/IPython tools dispatch through the provider under their existing Permissions checks and a refreshed current control generation. Computer bindings carry one run identity; persistent provider processes are bounded and fail closed without run-scoped cleanup, and Turn Loop requires cleanup before the Agent run can settle. Session Jobs persist a first-class `waiting-for-computer` active state with bounded node/reason context before Computer admission blocks, restore that wait metadata through restart reconstruction, and return to `running` when a screen is granted. Browser actions repeat Browser Supervisor/resource admission on the already-leased screen, so browser/renderer/GPU/CPU/RAM/screen pressure enters that same durable wait/resume path rather than failing later at dispatch. The Computer plugin contributes permission-gated `computer_observe`/`computer_browser`/`computer_visual_probe` Agent tools for the active leased screen. The model path uses bounded semantic elements, observation-scoped refs, structural deltas, stale-ref rejection, and uncertainty/high-impact gating before a bounded visual crop is allowed; visual pixel payloads are current-turn-only and are omitted from durable Session history. Authenticated Client Gateway APIs expose bounded status/node/screen/lease state plus observation and device-bound human takeover/activity/hand-back. Every provider observation must positively attest omission of secrets, keystrokes, CAPTCHA contents, and sensitive screenshots; Computer validates that attestation and applies defense-in-depth redaction before Agent or Client exposure. Human takeover pauses interrupted Agent Computer work without replaying it; hand-back records a fresh observation and wakes the same job/tool context at the new generation. The provider-neutral login-wall acceptance path now runs through the real Agent executor, Project target resolution, Permissions, Computer contributions, and Session Jobs while proving secret exclusion, fresh signed-in re-observation, same-Session-Job continuation, and no stale GUI replay; Turn Runtime detached-job admission is covered separately in the same integration suite. The implementation is complete in Phase 4 scope and the audit-hardening repository verification gate has passed; Linux native X11 and Windows platform implementations and their real-provider observation-safety conformance tests remain in Phases 5 and 8.
...
A Computer Node is shared by the user and Agents: one OS user, home directory, files, downloads, applications, and persistent browser profile/cache/login state. Each concurrently active Agent receives a separate leased screen. Root profile screen defaults may pin one screen; child Agents treat inherited profile defaults as soft preferences so siblings can use other free screens. Admission considers RAM, CPU, browser renderer count, GPU, and screen workload; overloaded work waits as `WAITING_FOR_COMPUTER` and resumes automatically.
...
Status: **complete for the native X11 Linux browser-control slice**. The built-in `linux-x11` provider treats real host virtual desktops as Human/Agent screens, reuses the hardened Chromium-family CDP engine for semantic perception/actions, and launches a real Brave/Chrome/Chromium window directly on an Agent desktop. The built-in source/deployment path contains only the native `linux-x11` provider; the retired hidden-compositor/viewer implementation and its deployment artifacts have been removed. Routine task approval survives durable retries of the same admitted turn/job, and managed-process cleanup remains valid after Agent async-local ownership unwinds. See [`operations/LINUX_COMPUTER.md`](operations/LINUX_COMPUTER.md).
...
Acceptance: a user can switch between their normal X11 virtual desktop and a FRIDAY desktop containing the real controlled browser window; one approved Telegram request does not generate repeated routine approvals when durable delivery retries, and successful browser work is reported instead of being converted into cleanup failure.
...
Acceptance: browser, shell, files, Git, UI Automation, and virtual-screen work run in parallel while raw visual input is safely serialized. The Windows provider must also pass the same Computer observation-safety conformance coverage for passwords, OTPs, human keystrokes, CAPTCHA contents, and sensitive screenshots before model-visible observation is enabled.
...
| 5 | None | `computer` Linux provider, Doctor, deployment | Phase 4 provider contract | Run native X11 virtual desktops with a persistent FRIDAY browser profile | Linux native-desktop smoke test passes |
...
| Linux native desktop services | They are deployment/provider code for Computer, not a new business domain | User-level browser service; X11/EWMH virtual desktops; persistent Brave/Chrome/Chromium profile |
...
| Native X11 desktop mode | 5 | No | Primary Linux deployment option |
| Raspberry Pi 4 | 5 | No | Requires a supported X11 desktop session for native visible Computer control |
...
| 13 | Phase 5: native X11 virtual desktops |
| 14 | Phase 5: native X11 browser/desktop lifecycle |
