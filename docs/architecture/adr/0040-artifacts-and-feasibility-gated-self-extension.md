...
1. verify the sandbox and a clean primary repository baseline;
2. verify an implementation model is available;
3. run a bounded, tool-free feasibility analysis;
4. only if feasible, tell the user that the feature is unavailable but can be
   built now;
