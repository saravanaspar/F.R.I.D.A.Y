...
The first command prints the generated capability/contribution/hook inventory plus public-type counts. The JSON form contains every configured ordinary plugin, every surface id/type binding, every exported ordinary public type, and the set reachable from semantic surfaces, so humans, CI, or another AI can audit discoverability without maintaining a hand-written list.
