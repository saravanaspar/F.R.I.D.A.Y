...
Every route uses the existing paired-device challenge/signature authentication. Status refreshes provider telemetry and returns bounded Computer-owned summaries; browser status includes readiness and counts but not tab URLs/titles. `/v1/computer/observe` derives the current screen-lease owner and control generation on the server and returns only an observation that has passed the mandatory provider model-safety attestation and Computer-core defense-in-depth redaction, including at most an Artifact reference for a screenshot rather than raw screenshot bytes. It fails closed if the provider cannot attest omission of secrets, keystrokes, CAPTCHA contents, or sensitive screenshots, while a human owns control, or after a generation change.
