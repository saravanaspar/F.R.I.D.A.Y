...
import { scopedCuaArguments } from "../plugins/cua/index.js";
...
describe("CUA Driver MCP connection", () => {
  it("isolates CUA browser sessions and ignores a model supplied session label", () => {
    const tool = { name: "get_browser_state", inputSchema: { type: "object", properties: { session: { type: "string" } } } };
    const first = scopedCuaArguments(tool, { session: "other-agent", pid: 42 }, { sessionId: "first", ownerScope: "operator" });
    const second = scopedCuaArguments(tool, { pid: 42 }, { sessionId: "second", ownerScope: "operator" });
    expect(first.session).toMatch(/^friday-[a-f0-9]{20}$/);
    expect(second.session).not.toBe(first.session);
    expect(scopedCuaArguments(tool, { pid: 42 }, { sessionId: "first", ownerScope: "operator" })).toEqual(first);
  });
